...
`BaseResponseObject.data`는 Swagger에 구체적 스키마 없이 `object`로만 선언돼 있다. 같은 데이터를 반환하는 공개 endpoint `GET /api/introductions`(`operationId: getIntroduction`, 인증 불필요)는 `IntroResponse`로 스키마가 명확해 `data`가 이 shape일 것으로 본다 — 구현 시점에 실제 admin 토큰으로 재확인한다(`.claude/known-issues/records/api-introductions-500.md` 참고, 이 공개 endpoint가 현재 500을 반환해 아직 실증 못함).

### `IntroResponse` (`data`로 추정)
...
응답은 snake_case, 요청(`IntroRequest`)은 camelCase다. `bannerCardinal`은 요청에만 있고 응답에는 없다 — 조회로 현재 기수를 복원할 수 없다.

`banner_status`/`banner_content`는 Figma·실제 홈페이지 기준 모집 배너 문구가 쓰이지 않는 것으로 확인돼 `IntroductionsPage` 구현 범위 밖이다. 응답 shape나 `bannerCardinal`/`bannerStatus`와의 관계는 추측하지 않는다.
